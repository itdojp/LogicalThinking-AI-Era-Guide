...
本書では「AI時代」を、特定モデルの仕様や一時的な流行語ではなく、
業務成果物を作るための思考・表現・検証の変化として扱います。
読み進める際は、次の観点を確認してください。

- 事実、仮説、推奨、要確認事項を分け、根拠のない数値効果を鵜呑みにしない。
- 生成AIへの指示は、目的、読み手、制約、出力形式、受け入れ基準とセットで設計する。
- AI出力は初稿であり、人間が根拠確認、編集、承認、説明責任を担う。
- テンプレートやケーススタディは、1枚サマリー、議事録、提案骨子などの成果物単位で再利用する。
