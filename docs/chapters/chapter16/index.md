...
**AIに任せる**
- 計画のたたき台（憲章、WBS、スケジュール案、リスク洗い出し）
- ステークホルダー整理（関係者の分類、論点と懸念の列挙）
- 進捗報告の下書き（事実の整理、要点化、次アクション提示）

**AIに任せない**
- リソース確保とコミット（予算、人員、契約、納期の確定）
- 変更承認（スコープ変更、優先順位変更の最終判断）
- 事実確定（公式な計画・合意内容の確定）

#### 章専用プロンプトテンプレ（CRISP-P）

```text
【背景】（プロジェクト名）を進めています。目的は（例: 新システム導入/業務改善）です。関係者は（例: 事業部/開発/運用/法務）です。
【役割】プロジェクトマネージャーとして
【指示】プロジェクト憲章の要点（目的、スコープ、成功基準、制約、主要リスク、ステークホルダー）を整理し、実行計画のたたき台を作成してください。
【制約】不確かな前提は質問してください。確定していない日付や数値は断定しないでください。
【成果物】A4 1枚のプロジェクト要約（Markdown）として出力してください。
```
...
- 章固有KPI（プロジェクト運用品質）
  - 目的・スコープ・成功基準・制約が明確で、関係者が同じ前提を共有できる
  - 主要リスクが列挙され、対策（予防/検知/復旧）が紐づいている
  - 変更が起きたときの判断軸（優先順位、影響範囲）が明示されている

#### NG例（典型）

- 目的や成功基準が曖昧なまま、スケジュールだけを作って進める
- 根拠のない工数・期日をAIが生成したまま採用する
- 合意が取れていない変更を、既成事実として進捗報告に混ぜる

#### 最小演習（10分）＋実務課題（30分）

**10分（リスク台帳）**
1. 自分のプロジェクト（または想定案件）で、リスクを5つ列挙する
2. AIに「影響度/発生確率/対策案」まで整理させ、現実性を人が確認する

**30分（1枚の計画要約）**
1. 憲章の要点（目的/スコープ/成功基準/制約/主要リスク）をA4 1枚にまとめる
2. 変更要求が来た想定で「影響・代替案・判断材料」を整理し、承認ゲートの形にする
...
この章ではプロジェクト管理と問題解決の論理的アプローチを学びました。

次の第17章では、これまで学んだすべての内容を統合し、継続的学習と思考力向上の仕組みについて学びます。個人の成長から組織の文化変革まで、論理的思考を継続的に発展させる方法を身につけましょう。
