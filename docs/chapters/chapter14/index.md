...
マルチモーダル活用では、画像やPDFの中に機密情報が埋もれることがあります。ファイル名だけで判断せず、画面内の氏名、ID、顧客名、契約条件、価格、内部URL、通知内容を確認します。
...
| 画面上に顧客IDが表示される | 事実 | スクリーンショット | 右上 | 2026-05-25 | 確認済み | 画像マスク | マスク漏れ |
```
...
スクリーンショットには、氏名、ID、内部URL、通知、顧客名、契約条件が映り込むことがあります。multimodal input inventory で分類し、必要ならマスクします。
