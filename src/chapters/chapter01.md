...
### 1.1.1 AI技術動向（陳腐化しやすい情報の扱い）

この節では、生成AIの動向を「変化の方向性」で整理します。モデル名・料金・制度の施行状況などは変化が速いため、最新情報は一次情報（公式発表）で要確認とします。

**大きな変化の方向性**
- **マルチモーダル化**: テキストに加え、画像・音声など複数形式を扱う
- **ツール連携の一般化**: 検索、データ処理、チケット管理など外部システムと組み合わせてタスクを完結させる
- **自律実行（エージェント化）**: タスクを分解し、手順を組み立てて反復的に進める使い方が増える
- **組織利用の前提が強まる**: 機密・監査・責任境界を定めないと運用できない

> 注記: 本書は「特定のモデル/ツールの最新仕様」を提供するものではありません。導入判断や運用設計に必要な事実関係は、利用するサービスの公式情報で確認してください。
...
**AI導入後の業務フロー（例）**
- 課題設定・文脈定義 → AIへの指示（必要に応じて画像等を含む） → 出力評価 → 反復改善 → 人間最終判断

**最新のAI協働パターン**
- **AIエージェント**: タスク分解・手順化を支援し、反復的に作業を進める
- **Human-in-the-Loop**: 継続的フィードバックによる品質向上
- **マルチモーダル**: テキスト・画像・音声など複数形式の統合処理
...
次の時間配分は、効果を保証する数値ではなく、作業の重心が移ることを示す例です。
実務では、題材の複雑さ、利用できる社内情報、承認フロー、品質基準により変動します。
